      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto grid max-w-full grid-cols-1 items-start gap-x-8 gap-y-12 sm:gap-y-16 lg:mx-0 lg:grid-cols-1">
                ...
                <div className="relative z-10 p-10">
                  ...
                  {contentInfo.quote && (
                    <blockquote className="text-base pt-10 sm:text-lg md:text-xl font-medium leading-relaxed text-white">
                      <svg
                        className="mb-3 h-6 w-6 sm:h-8 sm:w-8 text-red-400"
                        fill="currentColor"
                        viewBox="0 0 24 24"
                      >
                        <path d="M14.017 21v-7.391c0-5.704 3.731-9.57 8.983-10.609l.995 2.151c-2.432.917-3.995 3.638-3.995 5.849h4v10h-10zm-14.017 0v-7.391c0-5.704 3.748-9.57 9-10.609l.996 2.151c-2.433.917-3.996 3.638-3.996 5.849h4v10h-10z" />
                      </svg>
                      <p className="relative">{contentInfo.quote}</p>
                    </blockquote>
                  )}
          ...
          <div className="lg:pl-4">
            <div
              className={`text-sm sm:text-base leading-7 ${isDarkMode ? "text-slate-300" : "text-slate-700"}`}
            >
              ...
              {contentInfo.title && (
                <h2
                  className={`text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight ${isDarkMode ? "text-white" : "text-slate-900"}`}
                >
                  <span className="block">
                    {contentInfo.title.split(" ").slice(0, 2).join(" ")}
                  </span>
                  <span className="block text-red-600">
                    {contentInfo.title.split(" ").slice(2).join(" ")}
                  </span>
                </h2>
              )}
              ...
              {contentInfo.description && portableTextComponents && (
                <div
                  className={`mt-6 text-lg md:text-xl leading-8 text-gray-600 text-slate-600 ${isDarkMode ? "text-slate-400" : "text-slate-600"}`}
                >
                  <PortableText
            ...
            {contentInfo.stats && contentInfo.stats.length > 0 && (
              <div
                className={`mt-12 border-t ${isDarkMode ? "border-gray-800" : "border-gray-100"} pt-12`}
              >
                <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
                  {contentInfo.stats.map((stat, statIdx) => (
                    <div key={statIdx} className="text-center">
                      ...
                      <div
                        className={`mt-3 text-xs font-medium tracking-widest uppercase ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}
                      >
                        {stat.label}
                      </div>
                      <div
                        className={`mt-4 mx-auto h-0.5 w-12 ${isDarkMode ? "bg-gray-700" : "bg-gray-200"}`}
                      ></div>
                    </div>
                  ))}
                </div>
...
export {
  ...
  type ContentData,
  type EnhancedTestimonialSectionProps,
};
